          ...
          <div class="container">
            <form @submit="validateAndSubmit">
              ...
              <fieldset class="form-group">
                ...
                <input type="text" class="form-control" v-model="customer" :readonly="!isNew"/>
              </fieldset>
              <fieldset class="form-group">
                <label>Group list</label>
                <input type="text" class="form-control" v-model="groups" :readonly="!isNew"/>
              </fieldset>
              <fieldset class="form-group">
                <label>Vdn List</label>
                <input type="text" class="form-control" v-model="vdns" :readonly="!isNew"/>
              </fieldset>
            </form>
          </div>
        </div>
      </div>
      <p v-if="isNew">* 신규등록을 위해 1건 이상의 group 및 vdn을 등록하세요 </p>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  data() {
    return {
      ...
      errors: [],
      tableData: [
      { id: 1, customer: 'A사', vdns: '3678,4567', groups: 'A1,A2,A5', updateat: '2024-02-11 13:07:03' },
